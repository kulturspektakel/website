...
export function useNoiseStream({
  ...
  const ingest = useCallback<Ingest>(
    (deviceName, payload, receiveTime) => {
      ...
      try {
        ...
        console.error('[noise] decode error', e);
        return;
      }
  ...
  useEffect(() => {
    const client = mqtt.connect(BROKER_URL, {
      ...
      clientId: `kult-noise-${Math.random().toString(16).slice(2, 10)}`,
    });

    client.on('connect', () => {
      client.subscribe(TOPIC, {qos: 0}, (err) => {
        if (err) console.error('[noise] subscribe error', err);
      });
    });
    client.on('error', (e) => console.error('[noise] mqtt error', e));
