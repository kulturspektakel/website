...
export function parseCalibration(text: string): ParsedCalibration {
  ...
  if (
    ...
    return {
      ok: false,
      reason: `Expected ${CAL_BAND_COUNT} numbers, got ${parts.length}.`,
    };
  }
  ...
  useEffect(() => {
    ...
    const copy = () => {
      ...
      navigator.clipboard.writeText(formatCalibration(current)).then(
        () => toaster.create({type: 'success', title: 'Calibration copied'}),
        () => toaster.create({type: 'error', title: 'Copy failed'}),
      );
    };
    ...
    const paste = (text: string) => {
      ...
      if (!parsed.ok) {
        toaster.create({
          type: 'error',
          title: 'Calibration could not be pasted',
          description: parsed.reason,
        });
        return;
      }
      onPasteRef.current(parsed.offsets);
      toaster.create({type: 'success', title: 'Calibration pasted'});
    };
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (key === 'c') {
        ...
        navigator.clipboard
          .readText()
          .then(paste, () =>
            toaster.create({type: 'error', title: 'Paste failed'}),
          );
      }
    };
