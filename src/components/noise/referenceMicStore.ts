...
// Still `lautstaerke.` after the area was renamed to `noise`: this names bytes already
// sitting in crew members' browsers, not anything on screen. Renaming it would silently
// drop every calibration file anyone has dropped on the reference-mic panel, and the
// files come off a microphone that is not always to hand.
...
export function storeCalFile(store: CalStore, text: string): CalImport {
  ...
  if (parsed.serial == null) {
    return {
      ok: false,
      reason: 'No serial number in the file (the “SERNO:” line is missing).',
    };
  }
  if (parsed.cal.length < MIN_CAL_POINTS) {
    return {
      ok: false,
      reason: `Only ${parsed.cal.length} measurement point${parsed.cal.length === 1 ? '' : 's'} found — this does not look like a calibration file.`,
    };
  }
  return {
    ok: true,
