...
// An output is the same pair of strings — an id to hand back to an API, and something to put
// in a row — so this is an alias rather than a second shape. Named all the same, because a
...
/**
 * The outputs the pink noise could be played on.
 *
 * Same policy as the inputs above, and for the same two reasons — an empty id is one no API
 * will accept, and Chrome's `default`/`communications` aliases would list a device a second
 * time under a name that hides which one it is. The difference is that "follow the system" is
 * a real answer for an output, so the picker offers it as its own row rather than as one of
 * these: the empty sink id, which is the specification's way of saying it.
 */
export function audioOutputOptions(
  ...
  for (const d of devices) {
    ...
    options.push({
      deviceId: d.deviceId,
      label: d.label || `Output ${options.length + 1}`,
    });
  }
  return options;
}
...
/**
 * Whether this browser will let us say where the sound comes out.
 *
 * `AudioContext.setSinkId` is Chromium-only at the time of writing — Firefox and Safari have
 * neither it nor any other way to route Web Audio at a chosen device — so this is checked
 * rather than assumed, and the panel simply does not offer the choice where the answer is no.
 * That is not a degraded state worth explaining to anybody: the noise plays on whatever the
 * system is using, which is where it would have gone anyway.
 */
export const isOutputSelectable = (): boolean =>
