...
// The one formatter in the area that does *not* take the site's shared `locale`. Dates and
// numbers here stay German — this is a German festival, read on German phones — but a
// relative phrase is a sentence, and "vor 5 Minuten" set beside "Last seen" and "never seen"
// reads as a bug rather than as a locale. `localeCompare` below keeps `locale`: that sorts
// stage names, which are data.
 ...
 * to the callers that was `seen != null ? formatLastSeen(seen, now) : 'never seen'` copied
...
// Relative time (e.g. "5 minutes ago") for a past timestamp. Still exported, for the
