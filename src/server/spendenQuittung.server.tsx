...
// Split out of src/routes/api.spenden.quittung.$id.tsx so the route can reach it
// through `await import()`. Every route file is part of one shared server chunk,
// so a static import here put `@react-pdf/renderer` — and with it brotli,
// fontkit, pdfkit, yoga-layout and @react-pdf/reconciler — into the module graph
// that a plain page render compiles on every cold start, measured at ~213 ms for
// code only this one endpoint can reach. Nothing in this file is imported until
// somebody actually downloads a receipt.
...
export async function renderSpendenQuittung({
  donation,
  origin,
}: {
  donation: {
  ...
  const stream = await renderToStream(
    <Document language="de">
      <Page size="A4" style={styles.page}>
        <Image src={`${origin}/logos/logo-wide.png`} style={styles.logo} />
        ...
        <Text>
          ...
          1 Nr. 9 des Körperschaft- steuergesetzes bezeichneten Körperschaften,
        ...
        <Section label="Zuwendung:">
          <Text>
            ...
            (in Worten:{' '}
            {de(Math.floor(donation.amount / 100)).replace(/eins$/, 'ein')} Euro
            und {de(donation.amount % 100).replace(/eins$/, 'ein')} Cent)
          </Text>
          <Text>
            Tag der Zuwendung:{' '}
        ...
        <Text>
          ...
          Körperschaftsteuerbescheid des Finanzamtes Fürstenfeldbruck StNr.
          117/109/60900 vom 05.07.2024 nach § 5 Abs. 1 Nr. 9 des
          Körperschaftsteuergesetzes von der Körperschaftsteuer und nach § 3 Nr.
          6 des Gewerbesteuergesetzes von der Gewerbesteuer befreit.
        </Text>
        <Text>
          Es wird bestätigt, dass die Zuwendung nur zur Förderung der Kunst und
        ...
        <View style={styles.footer}>
          <View style={styles.footerCol}>
            <Text style={styles.footerTitle}>Kulturspektakel Gauting e.V.</Text>
            <View>
          ...
          <View style={styles.footerCol}>
            <View>
              <Text>Registergericht: Amtsgericht München</Text>
  ...
  return (Readable as any).toWeb(stream) as ReadableStream;
}
