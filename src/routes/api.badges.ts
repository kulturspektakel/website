  ...
  server: {
    handlers: {
      POST: async ({request}) => {
        ...
        const _crewCard = await queryCrewCard(
          new Uint8Array(crewCardId),
          event,
        );
        if (!_crewCard) {
        ...
        return json(
          newlyAwardedBadges.map(({badgeKey, awardedAt}) => ({
            ...
            availableForCrew: badgeConfig[badgeKey].availableForCrew,
            availableForRegular: badgeConfig[badgeKey].availableForRegular,
            awardedAt,
            svg: renderToString(
              React.createElement(BadgeSVG, {type: badgeKey}),
            ),
          })),
        );
      },
